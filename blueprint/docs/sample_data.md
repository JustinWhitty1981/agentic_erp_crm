...
This document provides sample data for testing the Agent First ERP CRM system. The data includes:
...
```sql
INSERT INTO agent_first_erp_crm.entities (entity_type, name, industry, website, status)
VALUES ('customer', 'Acme Corporation', 'Manufacturing', 'https://acme-corp.com', 'active');

-- Primary contact
INSERT INTO agent_first_erp_crm.contacts (first_name, last_name, email, phone, title, status)
VALUES ('John', 'Smith', 'john.smith@acme.com', '555-1234', 'Procurement Manager', 'active')
RETURNING id;

-- Link contact to entity
INSERT INTO agent_first_erp_crm.entity_relationships (entity_id, contact_id, role, is_primary)
VALUES (1, 1, 'Procurement Manager', true);

-- Primary address
INSERT INTO agent_first_erp_crm.addresses (entity_id, address_type, street, city, state, postal_code, country, is_primary)
...
```sql
INSERT INTO agent_first_erp_crm.entities (entity_type, name, industry, status)
...
```sql
INSERT INTO agent_first_erp_crm.item_categories (category_code, name) VALUES
('RAW', 'Raw Materials'),
('FIN', 'Finished Goods'),
('ACC', 'Accessories');

INSERT INTO agent_first_erp_crm.item_categories (category_code, name, parent_category_id)
SELECT 'SUB', 'Subcomponents', id FROM agent_first_erp_crm.item_categories WHERE category_code = 'RAW';
```

### Warehouses

```sql
INSERT INTO agent_first_erp_crm.warehouses (warehouse_code, name, is_active) VALUES
...
```sql
INSERT INTO agent_first_erp_crm.locations (warehouse_id, location_code, zone, aisle, bin)
SELECT w.warehouse_id, 'A-01-01', 'A', '01', '01'
FROM agent_first_erp_crm.warehouses w WHERE w.warehouse_code = 'WH-MAIN';
```

### Inventory Items

```sql
INSERT INTO agent_first_erp_crm.inventory_items (sku, name, category_id, base_unit_of_measure, standard_cost, safety_stock, reorder_point, is_active)
SELECT 'WIDGET-A', 'Widget Type A', c.category_id, 'EACH', 10.50, 100, 200, true
FROM agent_first_erp_crm.item_categories c WHERE c.category_code = 'FIN';
...
FROM agent_first_erp_crm.item_categories c WHERE c.category_code = 'RAW';
```

### Inventory On Hand

```sql
INSERT INTO agent_first_erp_crm.inventory_on_hand (item_id, warehouse_id, quantity_on_hand, quantity_available, quantity_reserved)
SELECT i.item_id, w.warehouse_id, 500, 450, 50
FROM agent_first_erp_crm.inventory_items i, agent_first_erp_crm.warehouses w
...
```sql
-- Check entities
SELECT entity_type, COUNT(*) FROM agent_first_erp_crm.entities GROUP BY entity_type;

-- Check communications
SELECT communication_type, COUNT(*) FROM agent_first_erp_crm.communications GROUP BY communication_type;

-- Check inventory
SELECT i.sku, i.name, ioh.quantity_on_hand, w.name as warehouse
FROM agent_first_erp_crm.inventory_items i
JOIN agent_first_erp_crm.inventory_on_hand ioh ON i.item_id = ioh.item_id
JOIN agent_first_erp_crm.warehouses w ON ioh.warehouse_id = w.warehouse_id;
```

---

*Generated for Agent First ERP CRM - Sample data setup documentation*