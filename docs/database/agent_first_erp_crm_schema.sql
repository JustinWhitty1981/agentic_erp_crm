...
--
-- Name: agent_interactions; Type: TABLE; Schema: -; Owner: -
--
...
--
-- Name: idx_actions; Type: INDEX; Schema: -; Owner: -
--

CREATE INDEX IF NOT EXISTS idx_actions ON agent_interactions USING gin (actions_taken);

--
-- Name: idx_conv; Type: INDEX; Schema: -; Owner: -
--

CREATE INDEX IF NOT EXISTS idx_conv ON agent_interactions (conversation_id);

--
-- Name: idx_timestamp; Type: INDEX; Schema: -; Owner: -
--

CREATE INDEX IF NOT EXISTS idx_timestamp ON agent_interactions ("timestamp" DESC);

--
-- Name: idx_user; Type: INDEX; Schema: -; Owner: -
--

CREATE INDEX IF NOT EXISTS idx_user ON agent_interactions (user_id);
...
--
-- Name: add_customer(text, text, text, text, text, text, text, text, text, text); Type: FUNCTION; Schema: -; Owner: -
--
...
--
-- Name: add_customer(text, text, text, text, text, text, text, text, text, text, text, text, text, boolean); Type: FUNCTION; Schema: -; Owner: -
--
...
DECLARE
    ...
    v_existing_phone_count INTEGER;
BEGIN
    -- Validation for individuals: Require first and last name
    IF p_entity_type = 'individual' THEN
        IF p_first_name IS NULL OR p_first_name = '' THEN
            RETURN QUERY SELECT FALSE, NULL::INTEGER, NULL::INTEGER, 'First name is required for individual customers';
            RETURN;
        END IF;

        IF p_last_name IS NULL OR p_last_name = '' THEN
            RETURN QUERY SELECT FALSE, NULL::INTEGER, NULL::INTEGER, 'Last name is required for individual customers';
            RETURN;
        END IF;
    END IF;

    -- Validation for businesses: Require organization name
    IF p_entity_type = 'business' THEN
        IF p_organization_name IS NULL OR p_organization_name = '' THEN
            RETURN QUERY SELECT FALSE, NULL::INTEGER, NULL::INTEGER, 'Organization name is required for business customers';
            RETURN;
        END IF;
    END IF;
    ...
    IF p_entity_type = 'business' THEN
        INSERT INTO agent_first_erp_crm.entities (entity_type, name, status, created_at, updated_at)
        VALUES ('customer', trim(p_organization_name), p_status, NOW(), NOW())
        RETURNING id INTO v_entity_id;
    ELSE
        INSERT INTO agent_first_erp_crm.entities (entity_type, name, status, created_at, updated_at)
        VALUES ('customer', trim(p_first_name || ' ' || p_last_name), p_status, NOW(), NOW())
    ...
    -- Create the contact
    IF p_entity_type = 'individual' OR (p_entity_type = 'business' AND (p_first_name IS NOT NULL OR p_last_name IS NOT NULL)) THEN
        INSERT INTO agent_first_erp_crm.contacts (first_name, last_name, email, phone, status, created_at, updated_at)
        VALUES (
    ...
    ELSE
        INSERT INTO agent_first_erp_crm.contacts (first_name, last_name, email, phone, status, created_at, updated_at)
        VALUES ('', '', p_email, p_phone, 'active', NOW(), NOW())
        RETURNING id INTO v_contact_id;

        INSERT INTO agent_first_erp_crm.entity_relationships (entity_id, contact_id, role, is_primary, created_at)
        VALUES (v_entity_id, v_contact_id, 'Business Contact', TRUE, NOW());
    END IF;
    ...
    IF p_entity_type = 'business' THEN
        RETURN QUERY SELECT TRUE, v_entity_id, v_contact_id,
            'Business customer added successfully: ' || p_organization_name;
    ELSE
        RETURN QUERY SELECT TRUE, v_entity_id, v_contact_id,
            'Customer added successfully: ' || p_first_name || ' ' || p_last_name;
    END IF;
...
--
-- Name: get_followup_customers(integer); Type: FUNCTION; Schema: -; Owner: -
--

CREATE OR REPLACE FUNCTION get_followup_customers(
    p_days_threshold integer DEFAULT 7
)
RETURNS TABLE(entity_id integer, name text, email text, phone text, last_communication_date timestamptz, follow_up_reason text)
LANGUAGE plpgsql
VOLATILE
AS $$
DECLARE
...
--
-- Name: update_customer(text, text, text, text, text, text, text, text, text, text, text, text, text); Type: FUNCTION; Schema: -; Owner: -
--
...
BEGIN
    -- Find the customer
    SELECT e.id, c.id INTO v_entity_id, v_contact_id
    FROM agent_first_erp_crm.entities e
    JOIN agent_first_erp_crm.entity_relationships er ON e.id = er.entity_id AND er.is_primary = TRUE
    JOIN agent_first_erp_crm.contacts c ON er.contact_id = c.id
    ...
    IF v_entity_id IS NULL THEN
        RETURN QUERY SELECT FALSE, NULL::INTEGER, NULL::INTEGER, 'Customer not found';
        RETURN;
    END IF;
    ...
    IF p_first_name IS NOT NULL OR p_last_name IS NOT NULL OR p_email IS NOT NULL OR p_phone IS NOT NULL THEN
    ...
    -- Update entity information if provided
    IF p_first_name IS NOT NULL OR p_last_name IS NOT NULL OR p_status IS NOT NULL THEN
        UPDATE agent_first_erp_crm.entities
        SET name = CASE
    ...
    -- Handle address updates with FIXED logic
    IF p_street IS NOT NULL OR p_city IS NOT NULL OR p_state IS NOT NULL OR p_postal_code IS NOT NULL OR p_country IS NOT NULL THEN
        -- Step 1: Try to update the primary address
        UPDATE agent_first_erp_crm.addresses addr
        SET street = COALESCE(p_street, addr.street),
        ...
        RETURNING addr.id INTO v_address_id;

        -- Step 2: If no primary address exists, check if any address exists
        IF v_address_id IS NULL THEN
            -- Find any existing address for this entity
            SELECT addr.id INTO v_address_id
            FROM agent_first_erp_crm.addresses addr
            WHERE addr.entity_id = v_entity_id
            LIMIT 1;

            -- If an address exists (even if not primary), update it and mark as primary
            IF v_address_id IS NOT NULL THEN
                ...
                WHERE addr.id = v_address_id;
            ELSE
                -- Step 3: No addresses exist, create a new one
                INSERT INTO agent_first_erp_crm.addresses (
                    entity_id, contact_id, address_type,
                    street, city, state, postal_code, country, is_primary, created_at
                ) VALUES (
                    v_entity_id, v_contact_id, 'home',
                    p_street, p_city, p_state, p_postal_code,
...
CREATE OR REPLACE VIEW customers AS
 SELECT e.id,
    ...
    c.email,
    c.phone,
    ( SELECT concat_ws(', '::text, a.street, a.city, a.state, a.postal_code) AS concat_ws
           FROM addresses a
